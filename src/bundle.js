 ...
 4. Create Popup - createPopup / create_item_template (getPopupDetails (switch)) / createItemHtml
	...
	function initNewsPageHover() {
		...
		if (pathnameArr.length > 5) {
			...
			btmButton = '<button id="btm-btn-' + slug + '" style="' + btn_primary_style + '" class="google-search btn btn-primary" href="javascript:void(0);" data-slug="' + slug + '">' + 'SHOW ALTERNATIVES' + '</button>';

			btmHover = '<div data-slug="' + slug + '" style="' + popover_style + ';position:fixed;' + side + ':50px;bottom:10px;">' + '<h3 style="' + popover_title_style + '">' + 'BRIDGE THE MEDIA' + '</h3>' + '<div id="btm-hover-' + slug + '">' + "<div style='max-height:450px;overflow:scroll;'id='btm-popover-body-" + slug + "'></div>" + btmButton + "</div>" + '</div>';
		}
	...
	$a.each(function (index, link) {
		...
		if (href) {
			...
			var content = '<button id="btm-btn-' + slug + '" style="' + btn_primary_style + '" class="google-search btn btn-primary" href="javascript:void(0);" data-slug="' + slug + '">' + 'SHOW ALTERNATIVES' + '</button><div id="btm-popover-body-' + slug + '"></div>';
